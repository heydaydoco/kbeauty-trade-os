# kbeauty-trade-os — WBS(세션 태스크 분해서) v1.0
...
**S0-1 | 리포 골격·인프라·CI**
- 범위: §2 ADR-01·02, §18.2·18.3, §17.1 골격
- 산출물: FastAPI+PostgreSQL+React(Vite·Tailwind) 모노레포 골격, Docker Compose dev/prod 분리, .env 분리, alembic 마이그레이션 체계, 헬스체크 엔드포인트, GitHub Actions CI(전체 테스트+마이그레이션 드라이런, 실패 시 병합 차단), 구조화 로깅(마스킹)
- DoD: `docker compose up` 한 번으로 dev 기동 / CI가 push에 실제로 돌고 실패 시 빨간불 / 헬스체크 200
- 검증: K(CI 실패 병합 차단), 스모크 테스트 1개 이상
...
- 산출물: comm_logs(폴리모픽)/agency_contracts/스코어카드, 정체 N일 알림, 전달 서류 zip, T1 템플릿 시드(**실제 진출 시장분만 투입**), 신규 시장 위저드 5단계, 장애 운영 SOP 문서+수기 양식
- DoD: 정체 N일 초과 → 독촉 알림 / 시드 데이터 전건 근거링크 보유
